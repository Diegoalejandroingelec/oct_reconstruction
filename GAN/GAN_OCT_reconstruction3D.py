...
class Discriminator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv3d(ndf, ndf * 2, kernel_size=3, stride=2,padding=[1,1,1]),
            nn.BatchNorm3d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            #nn.MaxPool3d(2),

            nn.Conv3d(ndf * 2, ndf * 4, kernel_size=3, stride=2,padding=[1,1,1]),
            nn.BatchNorm3d(ndf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            #nn.MaxPool3d(2),
            

            nn.Conv3d(ndf * 4, ndf * 8, kernel_size=3, stride=2,padding=[1,1,1]),
            nn.BatchNorm3d(ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
            #nn.MaxPool3d(2),
            
            nn.Conv3d(ndf * 8, ndf * 16, kernel_size=3, stride=2,padding=[1,1,1]),
