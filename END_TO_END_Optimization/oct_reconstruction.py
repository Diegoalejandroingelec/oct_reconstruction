...
def create_3D_mask(w1,w2,w3,w4,original_volume=None,create_with_motion=False):
    ...
    expected_dims=bigger_sub_volumes_dim
    band_width=176
...
def reconstruct_volume_batches(volume,reconstruction_model,speeds_generator,sub_volumes_dim,reconstruct_with_motion):
    ...
    big_mask= np.zeros(original_volume_dim)
    overlap_pixels_w=50
    d_end=0
...
def initialize_speeds_generator_model(model_path):
    ...
    summary(speeds_generator_model, bigger_sub_volumes_dim)
...
def evaluate_model(denoised_dataset_folder_path,
    ...
    for i,test_volume_path in enumerate(test_volume_paths):
        try:
            ...
            bigger_reconstruction,mask,sub_sampled_volume=reconstruct_volume_batches(original_volume,
                                                             reconstruction_model,
                                                             speeds_generator,
                                                             bigger_sub_volumes_dim,
